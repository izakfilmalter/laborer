...
let managedRt: ManagedRuntime.ManagedRuntime<
  ...
  never
>
let dataChannelContext: Context.Context<TerminalManager | PtyHostClient>
...
beforeAll(async () => {
  ...
  dataChannelContext = await managedRt.context()
...
describe('Terminal data channel over MessagePort', { timeout: 30_000 }, () => {
  it('receives PTY output data via data channel port', async () => {
    ...
    // Attach the data channel using the shared service context.
    const fiber = Effect.runForkWith(dataChannelContext)(
  ...
  it('sends replay payload and replayComplete when attaching to a revived terminal', async () => {
    ...
    await Effect.runPromiseWith(dataChannelContext)(
      Effect.gen(function* () {
