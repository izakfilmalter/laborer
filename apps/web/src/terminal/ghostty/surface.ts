/** biome-ignore-all lint/complexity/noExcessiveCognitiveComplexity: vendored from t3code (github.com/pingdotgg/t3code); keep verbatim so the tree can be re-synced */
/** biome-ignore-all lint/complexity/noVoid: vendored from t3code (github.com/pingdotgg/t3code); keep verbatim so the tree can be re-synced */
/** biome-ignore-all lint/complexity/useOptionalChain: vendored from t3code (github.com/pingdotgg/t3code); keep verbatim so the tree can be re-synced */
...
/** biome-ignore-all lint/style/noNestedTernary: vendored from t3code (github.com/pingdotgg/t3code); keep verbatim so the tree can be re-synced */
/** biome-ignore-all lint/suspicious/noBitwiseOperators: vendored from t3code (github.com/pingdotgg/t3code); keep verbatim so the tree can be re-synced */
/** biome-ignore-all lint/suspicious/noFocusedTests: vendored from t3code (github.com/pingdotgg/t3code); keep verbatim so the tree can be re-synced */

import { isMonospaceFamily } from '../ghostty-support/fonts'
import { isMacPlatform } from '../ghostty-support/platform'
import {
...
export function terminalScrollbarGeometry(
  ...
  if (trackHeight <= 0 || len <= 0 || maxOffset === 0) {
    return null
  }
  ...
  for (const match of extractTerminalLinks(wrappedLine.text)) {
    if (offset >= match.start && offset < match.end) {
      ...
      const endRow = endSegment
        ...
        : undefined
      if (!(startSegment && endSegment && startRow && endRow)) {
        return null
      }
      return {
        kind: match.kind,
...
export function clearPrimedTerminalCopyInput(
  ...
  if (primedSelection.length === 0 || input.value !== primedSelection) {
    return
  }
  input.value = ''
}
...
export class GhosttyTerminalSurface {
  ...
  async setFont(font: GhosttyTerminalFont): Promise<void> {
    ...
    if (this.disposed || epoch !== this.fontEpoch) {
      return
    }
  ...
  /**
   * The URL under a viewport point, for a host that owns the right-click menu.
   * Reads the same detection the pointer hover uses, so the menu offers exactly
   * the link a Cmd/Ctrl+click would have opened.
   */
  ...
  /**
   * The visible rows as text, from the last painted frame. Reads no WASM state
   * — asking the core for a fresh snapshot would consume the dirty-row set the
   * renderer diffs against — so it reports what is actually on screen.
   */
  viewportText(): readonly string[] {
    return this.snapshot?.rowData.map((row) => row.text) ?? []
  }
      ...
      if (event.shiftKey) {
        ...
        if (typeof clipboard?.writeText === 'function') {
          ...
          void Promise.resolve().then(() => {
            if (this.disposed || this.copyShortcutToken !== token) {
              return
            }
    ...
    if (isTerminalPasteShortcut(event)) {
      ...
      if (typeof clipboard?.readText === 'function') {
        ...
        void clipboard.readText().then(
          (text) => {
            if (this.disposed || this.pasteShortcutToken !== token) {
              return
            }
            this.pasteShortcutToken += 1
            if (text.length > 0) {
  ...
  private readonly onInput = (event: Event) => {
    ...
    if (this.composing || inputEvent.isComposing) {
      return
    }
  ...
  private setSelectionAutoscroll(delta: number): void {
    ...
    this.selectionScrollTimer = window.setInterval(() => {
      if (this.disposed || this.selectionScrollDelta === 0) {
        return
      }
      this.scrollViewport(this.selectionScrollDelta)
      const pointer = this.selectionPointer
      if (pointer) {
        this.extendSelectionTo(pointer.x, pointer.y)
      }
    }, 80)
  }
  ...
  private updateLinkModifier(
    ...
    if (active === this.linkModifierActive) {
      return
    }
  ...
  private readonly onScrollbarPointerUp = (event: PointerEvent) => {
    if (event.pointerId !== this.scrollbarPointerId) {
      return
    }
    event.preventDefault()
  ...
  private positionInput(): void {
    ...
    if (left === this.inputLeft && top === this.inputTop) {
      return
    }
    this.inputLeft = left
    this.inputTop = top
