...
/**
 * Font family helpers the vendored Ghostty surface depends on.
 *
 * Lifted from t3code's `apps/web/src/appearanceFonts.ts`, reduced to the
 * monospace probe the terminal surface needs. The appearance-settings half of
 * that module (font-size clamps, Local Font Access queries, CSS variable
 * application) is deliberately not vendored: it is bound to t3code's settings
 * contracts, and we have no equivalent surface yet.
 */
...
export function isMonospaceFamily(family: string): boolean {
  ...
  try {
    ...
    for (const variant of MONOSPACE_PROBE_VARIANTS) {
      ...
      if (!areFontAdvancesMonospace(advances)) {
        return false
      }
    }
    return true
