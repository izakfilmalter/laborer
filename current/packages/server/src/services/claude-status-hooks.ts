  ...
  while (true) {
    try {
      ...
      if (
        ...
        (Date.now() - lockStartedAt >= 1000 && recoveredStaleLock)
      ) {
        return null
      }
      if (Date.now() - lockStartedAt >= 1000) {
        // A killed hook process can leave its lock directory behind. Sequence
        // state is atomically renamed, so removing one stale lock is safe.
...
const hookEntry = (scriptPath: string, status: AgentStatus) => ({
  matcher: '',
  hooks: [
    {
      type: 'command',
