...
/** biome-ignore-all lint/complexity/useSimplifiedLogicExpression: vendored from t3code (github.com/pingdotgg/t3code); keep verbatim so the tree can be re-synced */
...
export function ghosttyTextRunEnd(
  ...
  while (end < cells.length) {
    ...
    if (next.text.length === 0 || !sameStyle(next)) {
      break
    }
    end += 1
  }
...
export function renderGhosttySnapshot(options: {
  ...
  for (const rowIndex of rowsToDraw) {
    ...
    for (let column = 0; column < row.cells.length; column += 1) {
      ...
      if (
        !cell ||
        !(cell.underline || cell.strikethrough || cell.overline || hoveredLink)
      ) {
        continue
      }
      ...
      if (cell.overline) {
        context.fillRect(left, top + 1, metrics.width, 1)
      }
    }
  }
