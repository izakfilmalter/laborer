...
- The development watcher covers TypeScript below `src/adapters`, `src/slack`, `src/acp-runtime`, and `src/prototype`. Changes to Cluster-host code, Action registrations, dependencies, environment, package metadata, Node, or the development supervisor require restarting `dev:slack`.
