 ...
 *   2. Authorize -> browser navigates to x-github-desktop-dev-auth://oauth?code=XXX&state=test
 *   3. Exchange the code:
...
// @github/alive-client references `self.location.origin` (browser global).
// Polyfill it for Node.js so AliveSession can construct.
...
describe('GitHub Alive with Desktop OAuth token', () => {
  it.skipIf(!DESKTOP_TOKEN)(
    ...
    async () => {
      ...
      // The test passes if we connected successfully — events are a bonus
