...
/** biome-ignore-all lint/style/useAtIndex: vendored from t3code (github.com/pingdotgg/t3code); keep verbatim so the tree can be re-synced */
...
/**
 * Terminal link detection for the vendored Ghostty surface.
 *
 * Vendored from t3code's `apps/web/src/terminal-links.ts`. This is deliberately
 * separate from `src/lib/terminal-links.ts`, which owns the host half of link
 * handling: opening a URL through the desktop bridge and the right-click menu.
 */
...
function collectMatches(
  ...
  for (const rawMatch of line.matchAll(pattern)) {
    ...
    if (kind === 'path' && isTerminalUrl(trimmed)) {
      continue
    }
