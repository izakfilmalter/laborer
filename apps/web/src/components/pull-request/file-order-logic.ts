/**
 * Diff files in reading order, ported from t3code's
 * `pullRequestFileOrder.logic.ts`: source in dependency order, then the
 * tests that cover it, then whatever a tool wrote. Ordering by path alone
 * buries the one file the change is really about under whichever
 * directory happens to sort first.
 */
import type { FileDiffMetadata } from '@pierre/diffs'
import { resolveFileDiffPath } from './diff-rendering-logic'
...
const GENERATED_NAME_PATTERN = /\.generated\./
const TEST_NAME_PATTERN = /\.(?:test|spec)\./
...
/** Resolves `../x` against the importer's directory; only what git shows. */
...
// all reduce to a quoted specifier preceded by one of three words.
  ...
  for (const line of lines) {
    ...
    while (match !== null) {
      ...
      const withExtension = specifier.startsWith('.')
        ...
        : specifier
      // A specifier may name the extension the module map dropped, and it
      // need not be the one on disk: TypeScript imports point at the `.js`
      // beside a `.ts`.
...
/**
 * Source files with what they import ahead of what imports them: Kahn's,
 * but every step takes the lowest remaining path, so the same change
 * always reads the same way. A cycle takes the lowest path anyway.
 */
...
/** The implementation a test names: `foo.test.ts` points at `foo`. */
...
/**
 * Diff files in reading order: source in dependency order, then the tests
 * that cover it, then whatever a tool wrote.
 */
export function orderDiffFiles(
  ...
  for (const file of files) {
    byPath.set(resolveFileDiffPath(file), file)
  }
  const tiers = new Map<string, DiffFileTier>()
  for (const path of byPath.keys()) {
    tiers.set(path, diffFileTier(path))
  }
  ...
  for (const base of ambiguousBaseNames) {
    byBaseName.delete(base)
  }

  const imports = new Map<string, ReadonlySet<string>>()
  for (const path of sourcePaths) {
    const file = byPath.get(path) as FileDiffMetadata
    imports.set(
      path,
      importedPaths(
        path,
  ...
  orderedSource.forEach((path, index) => {
    const base = baseName(path)
    if (!sourcePositions.has(base)) {
      sourcePositions.set(base, index)
    }
  })
  ...
  return [...orderedSource, ...orderedTests, ...orderedGenerated].map(
    (path) => byPath.get(path) as FileDiffMetadata
  )
}
