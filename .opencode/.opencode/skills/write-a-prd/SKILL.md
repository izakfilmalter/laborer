...
7. Once you have a complete understanding of the problem and solution, write the PRD content using the template below and save it using the `laborer_create_prd` MCP tool. Pass a concise title and the full markdown content.

If the PRD needs revisions after creation, use the `laborer_update_prd` MCP tool with the PRD's id and the updated content.

You can use `laborer_list_prds` to see existing PRDs and `laborer_read_prd` to read one by id or title.
