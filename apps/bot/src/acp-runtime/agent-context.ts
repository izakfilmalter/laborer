...
export const verifyAcpAgentContextSources = async (
  ...
  await verifyTrustedDirectoryIdentity(sources.rootDirectoryIdentity, operation)
...
const canonicalProspectiveGlobalRoot = async (
  ...
  try {
    await verifyRetainedDirectory(retained, 'resolve-global-agent-context-root')
...
const prepareMissingGlobalRootComponent = async (
  ...
  await assertNoSymlinkPathComponents(path, 'prepare-global-agent-context-root')
  const retained = await retainTrustedDirectory(
    path,
  ...
  try {
    ...
    await verifyRetainedDirectory(retained, 'prepare-global-agent-context-root')
  } finally {
    await retained.handle.close()
  }
}
    ...
    operation: async (locks) => {
      ...
      try {
        ...
        const sourceIsDuplicate =
          currentSource !== null && sameBytes(currentSource.bytes, staged.bytes)
        if (
          !stageIsUnchanged ||
