...
const observationPath = process.env.ACP_COMPATIBILITY_MCP_OBSERVATION
if (observationPath === undefined) {
  throw new Error('ACP_COMPATIBILITY_MCP_OBSERVATION is required')
}
...
const respond = async (request: JsonRpcRequest): Promise<void> => {
  ...
  if (request.method === 'initialize') {
    ...
    send({
      ...
      result: {
        ...
        serverInfo: {
          name: 'laborer-acp-compatibility-mcp',
          version: '1.0.0',
        },
      },
    })
  ...
  if (request.method === 'tools/list') {
    send({
      ...
      result: {
        tools: [
          {
            description: 'Records an ACP compatibility invocation.',
            inputSchema: {
  ...
  if (request.method === 'tools/call') {
    ...
    send({
      ...
      jsonrpc: '2.0',
      result: {
        content: [{ text: 'MCP fixture invocation recorded', type: 'text' }],
      },
    })
    return
  }
